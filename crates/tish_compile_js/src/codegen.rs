...
pub enum JsxMode {
    /// `h(tag, props, [children])` + `Fragment` (Tishact.tish). No `__h` preamble.
    #[default]
    TishactH,
    /// Historical `__h` + rest children (single-file snippets).
    LegacyDom,
    /// Vnode `__vdom_h` + `window.__tishactVdomPatch` (Tishact createRoot must use patch).
    Vdom,
}
            ...
            match self.jsx_mode {
                JsxMode::LegacyDom => {
                    let inline_h = format!(
                        ...
                        tish_jsx_web::LEGACY_DOM_PREAMBLE
                    );
                    self.output = self
                        .output
