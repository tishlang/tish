...
mod tests {
    ...
    /// #295: `--target js --format bundle` must re-emit the entry module's named exports as a real ES
    /// `export { … }` (previously only `export default` surfaced; `export fn/const/{…}` were silently
    /// dropped, forcing downstream regex post-processing of every bundle).
    #[test]
    fn bundle_emits_named_exports() {
        // `tempfile::tempdir()` (RAII, unique dir) rather than a fixed `std::env::temp_dir()` path.
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("entry.tish");
        ...
        let js = compile_project_with_jsx(&path, Some(dir.path()), false)
            .expect("compile_project_with_jsx failed");
        assert!(
