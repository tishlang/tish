...
//!
//! The intended contract (tishlang/tish#712):
//!
//! * `get` must validate the stored client id — a handle prepared on one
//!   client must never resolve against another. `close()` frees CLIENTS slab
//!   indices for reuse, so without this check a stale handle is replayed
//!   against whichever new connection reused the index.
//! * `prepare` must intern on `(client_id, sql)` so repeated prepares of
//!   identical SQL reuse one entry — bounding the table by program text
//!   instead of call count.
//! * `close(client)` must sweep that client's entries.
//! * `remove` (the `unprepare` export) must free a single entry.
//!
//! NOTE: this extraction currently reproduces the status-quo behavior the
//! issue describes (insert-only slab, client id ignored, no interning); the
//! tests below encode the intended contract and fail until the follow-up
//! commit implements it.
#![allow(dead_code)] // wired into `tish_sync` by the follow-up fix commit
...
impl<S: Clone> StatementRegistry<S> {
    ...
    ///
    /// Status quo: every call inserts a fresh entry — `sql` is accepted for
    /// the `(client_id, sql)` interning contract but not yet consulted
    /// (`Client::prepare` does not dedupe, and neither do we).
    pub(crate) fn insert(&mut self, client_id: usize, _sql: &str, stmt: S) -> usize {
        self.slab.insert((client_id, stmt))
    }

    /// Resolve a statement handle on behalf of `client_id`.
    ///
    /// Status quo: the stored client id is ignored (this reproduces the
    /// `let Some((_cid, stmt))` discard in `query_prepared` / `query_all`),
    /// so a stale handle resolves for whatever client the caller passes.
    pub(crate) fn get(&self, stmt_id: usize, _client_id: usize) -> Option<S> {
        self.slab.get(stmt_id).map(|(_cid, s)| s.clone())
    }

    /// Free one statement entry (the `unprepare` export).
    ///
    /// Status quo: no removal path exists — the slab is insert-only.
    pub(crate) fn remove(&mut self, _stmt_id: usize) -> Option<S> {
        None
    }

    /// Drop every entry belonging to `client_id`; returns how many were
    /// removed (the `close()` sweep).
    ///
    /// Status quo: `close()` sweeps CLIENTS only; statements are never swept.
    pub(crate) fn sweep_client(&mut self, _client_id: usize) -> usize {
        0
    }

    /// Find the interned handle for `(client_id, sql)`, if one exists.
    ///
    /// Status quo: nothing is interned, so nothing is ever found.
    pub(crate) fn lookup(&self, _client_id: usize, _sql: &str) -> Option<usize> {
        None
    }
...
mod tests {
    ...
    fn reprepare_same_sql_reuses_interned_entry() {
        ...
        assert_eq!(second, first, "identical (client, sql) must reuse the entry");
        assert_eq!(r.len(), 1);
