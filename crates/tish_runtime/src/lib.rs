...
/// `matmul_f32(N)` — tiled MSL compute kernel on the Metal GPU.
/// Returns `{ms, check}` as a `Value::Object`.
#[cfg(feature = "metal")]
pub fn metal_matmul_f32(args: &[Value]) -> Value {
    let n = match args.first() {
        Some(Value::Number(n)) => *n as usize,
        _ => return Value::String("metal_matmul_f32: expected number arg".into()),
    };
    match tish_metal::matmul_f32(n) {
        Ok((ms, check)) => make_result_obj(ms, check),
        Err(e) => Value::String(e.into()),
    }
}

/// `device_name()` — returns the Metal device name (e.g. "Apple M3 Pro").
#[cfg(feature = "metal")]
pub fn metal_device_name(_args: &[Value]) -> Value {
    match tish_metal::device_name() {
...
/// `matmul_f32(N)` — MLX matmul dispatched to the Metal GPU.
/// Returns `{ms, check}` as a `Value::Object`.
#[cfg(feature = "mlx")]
pub fn mlx_matmul_f32(args: &[Value]) -> Value {
    let n = match args.first() {
        Some(Value::Number(n)) => *n as usize,
        _ => return Value::String("mlx_matmul_f32: expected number arg".into()),
    };
    match tish_mlx::matmul_f32(n) {
        Ok((ms, check)) => make_result_obj(ms, check),
        Err(e) => Value::String(e.into()),
    }
}

/// `device_name()` — returns "Metal (Apple Silicon)".
#[cfg(feature = "mlx")]
pub fn mlx_device_name(_args: &[Value]) -> Value {
    Value::String(tish_mlx::device_name().into())
}

/// `version()` — returns the mlx-rs crate version.
#[cfg(feature = "mlx")]
pub fn mlx_version(_args: &[Value]) -> Value {
    Value::String(tish_mlx::version().into())
}
