...
**Type-only syntax:** `type Name = TypeExpr`; **`interface Name { k: T, … }`** (desugars to `type Name = { … }` — structural, lowers to a native struct on the Rust backend, checked by the gradual checker; `interface X extends Y { … }` desugars to an intersection of the parents with the body); `declare let` / `declare const`; `declare function` (no body). Function types use **`=>`**. These forms are erased at runtime in **`tish run`** and bytecode backends. *(The `tree-sitter-tish` editor grammar does not yet parse any type syntax — annotations, `type`, `declare`, `interface` — a separate gap from the compiler/LSP, which do.)*
